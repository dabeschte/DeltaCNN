...
__device__ __forceinline__ void load_mask(const int tile_start_in_y, const int tile_start_in_x, const int lane_idx, const uint32_t* batch_mask, uint32_t* s_mask, uint64_t& t_mask, uint32_t& density, const Dimensions& dim, const ConvConfig& config) {
    ...
    for (int i = threadIdx.x; i < n_in_px; i += BLOCK_SIZE) {
        ...
        //     const bool valid = y >= 0 && y < dim.in.h && x >= 0 && x < dim.in.w;
        //     printf("tId=%i i=%i mask_y=%i mask_x=%i valid=%i value=%i, t0y=%i t0x=%i\n", 
        //         threadIdx.x, i, y, x, (valid?1:0), (valid?batch_mask[mask_idx]:0), tile_start_in_y, tile_start_in_x);
        // }
...
__device__ __forceinline__ void write_mask_deconv(uint32_t* out_mask, const int batch, const uint64_t t_mask, const uint32_t *s_mask, const int density, const int tile_start_out_y, const int tile_start_out_x, const Dimensions &dim, const ConvConfig &config) {
    ...
    for (int out_px = threadIdx.x; out_px < n_pixels_out; out_px += BLOCK_SIZE) {
        ...
        if ((n_in_px <= 64 && t_mask != 0LLU) || (n_in_px > 64 && density != 0)) {
            ...
            if (KERNEL_SIZE == 1) {
                ...
                for (int y = 0; y <= KERNEL_SIZE; y++) {
                    for (int x = 0; x <= KERNEL_SIZE; x++) {
                        ...
                        // if (blockIdx.x == 2 && blockIdx.y == 0 && blockIdx.z == 0) {
                        //     printf("tId=%i p_y=%i p_x=%i out_y=%i out_x=%i y=%i x=%i mask_y=%i mask_x=%i mask_idx=%i set=%i, full_mask=%llu\n", 
                        //         threadIdx.x, p_y, p_x, out_y, out_x, y, x, mask_y, mask_x, mask_idx, (is_px_mask_set<w_in, n_in_px>(mask_x, mask_y, t_mask, s_mask)?1:0), t_mask);
                        // }
            ...
            // if (blockIdx.x == 2 && blockIdx.y == 0 && blockIdx.z == 0) {
            //     printf("tId=%i p_y=%i p_x=%i out_y=%i out_x=%i updated=%i\n", 
            //         threadIdx.x, p_y, p_x, out_y, out_x, updated);
            // }
...
__global__ void delta_deconv_sp(
    ...
    if (out_mask != nullptr && tile_start_z == 0) {
        ...
        //             printf("bId=%i mask[%i][%i]=%i\n", blockIdx.x, y, x, s_mask[y*w_in+x]);
        //         }
        //     }
        // }
                ...
                for (int in_x = IN_OFF; in_x < w_in + IN_OFF; ++in_x) {
                    ...
                    for (int f_y = min_f_y; f_y < max_f_y; ++f_y) {
                        ...
                        for (int f_x = min_f_x; f_x < max_f_x; ++f_x) {
                            ...
                            //     printf("in_y=%i in_x=%i f_y=%i f_x=%i min_f_y=%i max_f_y=%i min_f_x=%i max_f_x=%i, out_y=%i out_x=%i, val=%f, t_out_idx=%i t_out=%f\n",
                            //         in_y, in_x, f_y, f_x, min_f_y, max_f_y, min_f_x, max_f_x, in_y*STRIDE+f_y, in_x*STRIDE+f_x, val, t_out_idx, t_out[t_out_idx]
                            //     );
                            // }
                        }
                    }
        ...
        for (int in_c_off = 0; in_c_off < dim.in.c; in_c_off += WARP_SIZE) {
            ...
            if (dim.in.c == 3) {
                ...
                for (int px_idx = warp_idx; px_idx < n_in_px; px_idx += n_warps) {
                    ...
                    // if (blockIdx.x+blockIdx.y+blockIdx.z == 14 && in_c == 0) {
                    //     const int in_y_im = in_y * DILATION_Y + tile_start_in_y;
                    //     const int in_x_im = in_x * DILATION_X + tile_start_in_x;
                    //     printf("in_y=%i in_x=%i in_y_im=%i in_x_im=%i idx=%i valid=%i val=%f, warp_idx=%i, px_idx=%i, tId=%i, mask=%i\n",
                    //         in_y, in_x, in_y_im, in_x_im, in_y * w_in + in_x, (valid?1:0), (valid?batch_in[in_y_im * in_row_vals + in_x_im * dim.in.c + in_c]:0.0f),
                    //         warp_idx, px_idx, threadIdx.x, (is_px_mask_set<w_in, n_in_px>(in_x, in_y, t_mask, s_mask)?1:0)
                    //     );
                    // }
